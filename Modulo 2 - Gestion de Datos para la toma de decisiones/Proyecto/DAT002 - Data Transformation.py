...
const_path = r'C:\git\cenfotec\Modulo 2 - Gestion de Datos para la toma de decisiones\Proyecto\Datasets\IMDB Datasets'
...
def load_kaggle_data():
    ...
    titles_raw = pd.read_csv(
        const_path+r"\imdb-5000-movie-dataset.zip"
        , sep=','
        , dtype={'startYear': object}
        , compression='zip'
    )
    movie_titles = titles_raw  # [titles_raw['titleType'] == 'movie']
    movie_titles['movie_imdb_link'] = 'tt' + movie_titles['movie_imdb_link'].map(
        lambda x: x.lstrip(r'http://www.imdb.com/title').rstrip('?ref_=fn_tt_tt_1')
    )
    movie_titles['movie_imdb_link'] = movie_titles['movie_imdb_link'].map(
        lambda x: x.rstrip('/')
    )
    movie_titles = movie_titles.rename(columns={'movie_imdb_link': 'tconst'})
    print("Titles Kaggle loaded")
    return movie_titles

def load_title_data(kaggle_data):
    ...
    movie_titles = titles_raw[titles_raw['titleType'] == 'movie']
    movie_titles = pd.merge(movie_titles, kaggle_data[['tconst', 'country']], on='tconst')
...
def clean_movie_data(movie_titles):
    ...
    movie_titles_df = movie_titles[['tconst', 'primaryTitle', 'startYear', 'runtimeMinutes', 'country']]
    movie_titles_df = movie_titles_df.replace(r'\N', 'N/A')
    movie_titles_df = movie_titles_df.replace(r'NaN', 'N/A')
    movie_titles_df.columns = ['title_id', 'movie_name', 'year', 'runtime', 'country']
    movie_titles_df = movie_titles_df.drop_duplicates(subset='title_id')
...
def main():
    con = lite.connect('imdb_movies_pt2.db')
    cur = con.cursor()
    kaggle_data = load_kaggle_data()
    movie_data = load_title_data(kaggle_data=kaggle_data)
